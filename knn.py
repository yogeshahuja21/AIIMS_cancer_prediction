...
def predictKNN(x_train, y_train, x_test, y_test, KNN):
    y_pred = []
    # print("idhr aa gya")
    # print("x_test ka shape [0]",x_test.shape[0])
...
def plotting(inputDF, outputDF):
    ...
    for index, ax in enumerate(axes):  # enumerate is usefuls as , axes[index] = (ax)ith
        ...
        bins = (
            np.arange(start=inputDF.iloc[:, index].min(), stop=inputDF.iloc[:, index].max() + binwidth, step=binwidth))
...
def KNNplot(inputDF, outputDF):
    ...
    inputDF = inputDF.filter(items=['radius_mean', 'concavity_mean'],
                             axis=1)  # .filter takes out given rows/col. accordingly
    print(inputDF.shape)

    # sns.pairplot(inputDF)
    # plt.show()

    x_train, x_test, y_train, y_test = train_test_split(inputDF, outputDF, test_size=0.2)
